# coding: utf-8
from BayesSpamDetection import spamEmailBayes
import re
#spam类对象
spam=spamEmailBayes()
#保存词频的词典
...
normFileList=spam.get_File_List(r"./mail_data/normal")
spamFileList=spam.get_File_List(r"./mail_data/spam")
testFileList=spam.get_File_List(r"./mail_data/test")
#获取训练集中正常邮件与垃圾邮件的数量
...
for fileName in normFileList:
    wordsList.clear()
    for line in open("./mail_data/normal/"+fileName):
        #过滤掉非中文字符
...
for fileName in spamFileList:
    wordsList.clear()
    for line in open("./mail_data/spam/"+fileName):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
...
for fileName in testFileList:
    ...
    wordsDict.clear()
    wordsList.clear()
    for line in open("./mail_data/test/"+fileName):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
