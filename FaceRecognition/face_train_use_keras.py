...
from load_and_label_face_dataset import load_dataset, resize_image, IMAGE_SIZE
...
class DataSet(object):
    ...
    关于预处理，我们做了几项工作：
        ...
        4）归一化图像数据
    """
    # 加载数据集并按照交叉验证的原则划分数据集并进行相关的预处理工作
...
if __name__ == '__main__':
    dataset = DataSet('/tmp/face_recognition/data/')
    dataset.load()
    ...
    # 测试训练函数的代码
    model.train(dataset)
    model.save_model(file_path='/tmp/face_recognition/model/me.face.model.h5')
    K.clear_session()
    """
    ...
    #评估模型
    model = Model()
    model.load_model(file_path = '/tmp/face_recognition/model/me.face.model.h5')
